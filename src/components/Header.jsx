...
const Header = () => {
  ...
  return (
    <header 
      className={`fixed w-full z-50 transition-all duration-300 ${(isOpen || scrolled) ? 'bg-white shadow-md' : 'bg-transparent'}`}
    >
      <div className="container mx-auto px-4 py-3">
        <div className="flex gap-40 items-center mx-auto">
          {/* Logo */}
          <div className={`text-4xl font-dancing tracking-wider ${(isOpen || scrolled) ? 'text-rose-500' : 'text-rose-300'} hover:text-rose-400 transition-colors duration-300`}>
          ...
          <nav className="hidden md:flex space-x-10">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                <Link
                  to={link.path}
                  className={`text-xl font-sans transition-all duration-300 ${
                    (isOpen || scrolled) ? 'text-gray-700' : 'text-white'
                  } group-hover:text-rose-300 relative`}
                >
                  {link.name}
